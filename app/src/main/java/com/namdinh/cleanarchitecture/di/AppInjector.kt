...
package com.namdinh.cleanarchitecture.di
...
object AppInjector {
    fun init(cleanArchitectureApp: CleanArchitectureApp) {
        DaggerAppComponent.builder().application(application = cleanArchitectureApp)
                .serverAddress(serverAddress = AppCredentials.SERVER_ADDRESS)
                .realmModule(RealmModule(application = cleanArchitectureApp))
                .build().inject(cleanArchitectureApp)
